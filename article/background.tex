...
%This general approach suffers from methodological shortcomings that will be outlined in detail below, as well as insufficient documentation and quality control, as they are typically implemented ad hoc.
%Based on current practices we propose an improved solution, which includes rigorous QC.
...
%%% !!! Should I really go into detail on why exactly changing the data size is bad? In the background?
%%% Analogy to marker in microscopy
...
%%% !!! Mention resolution issue separately, only when histological info is needed
