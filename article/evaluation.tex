...
Volume conservation is based on the assumption that the total volume of the scanned segment of the brain should remain approximately identical after preprocessing.
...
\py{pytex_subfigs(
        ...
        caption='
                \\textbf{SAMRI Generic preprocessing reduces spatial smoothing, thereby increasing locality of functional analysis results.}
...
The next aspect of preprocessing quality we consider is the resulting image smoothness.
...
An additional way to assess preprocessing quality focuses on the robustness to variability resulting from repeat experimentation, and whether this is attained without compromising physiologically meaningful variability (i.e. overfitting).
...
For all three metrics, in the case of the Legacy workflow we find that the F-statistic is consistently higher for the session than for the subject variable:
