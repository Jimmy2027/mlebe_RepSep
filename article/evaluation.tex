...
\py{pytex_subfigs(
        ...
        caption='
                Volume change relative to the original scan volume.
...
\py{pytex_subfigs(
        ...
        caption='
                Mean per-animal significance distributions.
...
\py{pytex_fig('scripts/variance_catplot.py',
        ...
        label='var',
        caption='Metric scores for the similarity of resulting images with respect to the corresponding workflow template, plotted across subjects (separated into x-axis bins) and sessions (individual points in each x-axis bin).',
        multicol=True,
        )}
\end{sansmath}
...
Looking at \cref{fig:scf}, we note that the SCF is sensitive to both the processing workflow (\py{boilerplate.fstatistic_smoothness('Processing', condensed=True)}) and the template (\py{boilerplate.fstatistic_smoothness('Template', condensed=True)}), as well as their interaction (\py{boilerplate.fstatistic_smoothness('Processing:Template', condensed=True)}).
