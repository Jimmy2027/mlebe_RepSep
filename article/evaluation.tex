...
\py{pytex_subfigs(
        [
                {'script':'scripts/f_violin.py', 'label':'fv', 'conf':'article/1col.conf', 'options_pre':'{.48\\textwidth}',
			'caption':'Comparison across workflows and target templates, considering both BOLD and CBV both functional contrasts.'
                        ,},
                {'script':'scripts/fc_violin.py', 'label':'fcv','conf':'article/1col.conf', 'options_pre':'{.48\\textwidth}',
                        'caption':'Comparison across workflows and functional contrasts, considering only matching template-workflow combinations.'
                        ,},
                ],
        caption='Mean per-animal significance. Coloured patch width estimates distribution density, while continuous markers indicate the sample mean and dashed markers indicate the inner quartiles.',
        label='fig:vc',)}
\end{sansmath}

As seen in \cref{fig:fv}, we note that VCF is sensitive to both
the processing workflow (\py{boilerplate.fstatistic('Processing', dependent_variable='Mean Significance', df_path='data/functional.csv', condensed=True)}),
the template (\py{boilerplate.fstatistic('Template', dependent_variable='Mean Significance', df_path='data/functional.csv', condensed=True)}),
and interaction thereof (\py{boilerplate.fstatistic('Processing:Template', dependent_variable='Mean Significance', df_path='data/functional.csv', condensed=True)}).

Testing the core hypothesis of the comparison ---
whether the Generic SAMRI workflow (with the Generic template) performs significantly different than the Legacy workflow (with the Legacy template) ---
we note that it does
(two-tailed p-value of \py{pytex_printonly('scripts/f_t.py')}).

Descriptively, we observe that the effect with the greatest magnitude is that of the processing workflow variable, with its Legacy level introducing a notable volume loss
(VCF of \py{boilerplate.factorci('Processing[T.Legacy]', dependent_variable='Mean Significance', df_path='data/functional.csv')}).
...
\py{pytex_subfigs(
        [
                {'script':'scripts/fdr_violin.py', 'label':'fdrv', 'conf':'article/1col.conf', 'options_pre':'{.48\\textwidth}',
			'caption':'Comparison across workflows and target templates, considering both BOLD and CBV both functional contrasts.'
                        ,},
                {'script':'scripts/fdrc_violin.py', 'label':'fdrcv','conf':'article/1col.conf', 'options_pre':'{.48\\textwidth}',
                        'caption':'Comparison across workflows and functional contrasts, considering only matching template-workflow combinations.'
                        ,},
                ],
        caption='Mean per-animal significance in Dorsal Raphe. Coloured patch width estimates distribution density, while continuous markers indicate the sample mean and dashed markers indicate the inner quartiles.',
        label='fig:vc',)}
\end{sansmath}
...
\py{pytex_subfigs(
        [
                {'script':'scripts/fdrt_violin.py', 'label':'fdrv', 'conf':'article/1col.conf', 'options_pre':'{.48\\textwidth}',
			'caption':'Comparison across workflows and target templates, considering both BOLD and CBV both functional contrasts.'
                        ,},
                {'script':'scripts/fdrtc_violin.py', 'label':'fdrcv','conf':'article/1col.conf', 'options_pre':'{.48\\textwidth}',
                        'caption':'Comparison across workflows and functional contrasts, considering only matching template-workflow combinations.'
                        ,},
                ],
