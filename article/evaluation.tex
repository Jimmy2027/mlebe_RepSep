...
Functional analysis is another viable avenue for examining registration quality, as the metric being maximized in the registration process is not the same metric used for QC.
This method is however primarily suited to examine workflow effects in light of higher-level applications, and less suited for wide-spread QC (as it is the most computationally intensive and only applicable to a subset of all mouse MRI data).
As a first step we examine statistical power, estimated via the negative logarithm of p-value maps.
We compute the per-scan average of these values, which we term Mean Significance --- as seen in \cref{eq:ms}, where $n$ represents the number of voxel statistical estimates in the scan, and $p$ is a p-value.
...
Overall statistical power is, however, independent of the mapping quality, and functional analysis effects can be further inspected by visualizing statistical maps.
