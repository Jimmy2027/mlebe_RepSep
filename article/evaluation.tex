...
Looking at \cref{fig:sc}, we note that the SCF is sensitive to both the processing workflow (\py{boilerplate.fstatistic_smoothness('Processing', condensed=True)}) and the template (\py{boilerplate.fstatistic_smoothness('Template', condensed=True)}), as well as their interaction (\py{boilerplate.fstatistic_smoothness('Processing:Template', condensed=True)}).
The SCF is significantly different from 1 after processing with both the Legacy ($p= $\py{pytex_printonly('scripts/scf_t_legacy.py')}) and the Generic workflow ($p= $\py{pytex_printonly('scripts/scf_t_generic.py')}).
