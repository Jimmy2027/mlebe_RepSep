...
As seen in \cref{fig:fa}, we note that functional score is sensitive to both
the processing workflow (
\py{boilerplate.fstatistic('Processing', dependent_variable='Mean DR t', df_path='data/functional_t.csv', exclusion_criteria={'Subject':[4003,4006,4013]}, condensed=True)}
), the template (
\py{boilerplate.fstatistic('Template', dependent_variable='Mean DR t', df_path='data/functional_t.csv', exclusion_criteria={'Subject':[4003,4006,4013]}, condensed=True)}
), and interaction thereof (
\py{boilerplate.fstatistic('Processing:Template', dependent_variable='Mean DR t', df_path='data/functional_t.csv', exclusion_criteria={'Subject':[4003,4006,4013]}, condensed=True)}
).
...
\py{pytex_printonly('scripts/fa_t.py')}
).
...
\py{boilerplate.corecomparison_factorci('Contrast[T.CBV]', df_path='data/functional_t.csv', dependent_variable='Mean DR t', exclusion_criteria={'Subject':[4003,4006,4013]})}
).
...
(VCF of
\py{boilerplate.corecomparison_factorci('Processing[T.Legacy]:Contrast[T.CBV]', df_path='data/functional_t.csv', dependent_variable='Mean DR t', exclusion_criteria={'Subject':[4003,4006,4013]})}
).
...
\py{pytex_subfigs(
	[
		{'script':'scripts/map_generic.py', 'label':'mgd', 'conf':'article/map.conf', 'options_pre':'{.48\\textwidth}',
			'caption':'
				Note the origin at Bregma which provides histologically meaningful coordinates.
                                '
			,},
		{'script':'scripts/map_generic_ambmc.py', 'label':'mga','conf':'article/map.conf', 'options_pre':'{.48\\textwidth}',
			'caption':'
                                The “Legacy” template, which exemplifies histological contrast, the canonical histological template data matrix orientation (shared e.g. by the Allen Brain Institute template).
                                '
			,},
		{'script':'scripts/map_legacy_dsurqec.py', 'label':'mld','conf':'article/map.conf', 'options_pre':'{.48\\textwidth}',
			'caption':'
                                The “Legacy” template, which exemplifies histological contrast, the canonical histological template data matrix orientation (shared e.g. by the Allen Brain Institute template).
                                '
			,},
		{'script':'scripts/map_legacy.py', 'label':'mla','conf':'article/map.conf', 'options_pre':'{.48\\textwidth}',
			'caption':'
                                The “Legacy” template, which exemplifies histological contrast, the canonical histological template data matrix orientation (shared e.g. by the Allen Brain Institute template).'
			,},
		],
	caption='
                Multiplanar depictions of mouse brain templates, with the corresponding second-level omnibus statistical maps evaluating all CBV scans, and thresholded at $\mathrm{|t|\geq2}$
                (a low threshold is chosen so as to best depict both statistic values and the spatial statistic coverage).
