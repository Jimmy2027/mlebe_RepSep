...
\py{pytex_subfigs(
	...
	caption='
                Multiplanar depictions of the mouse brain templates, with the corresponding second-level omnibus statistical maps for all CBV scans, thresholded at $|t|\geq2$.
                A low threshold is chosen so as to best depict both the statistic and the statistic coverage.
                Note also that the statistic coverage is slightly underestimated, as the omnibus contrast is only defined for voxels captured in every functional scan. 
                ',
	label='fig:m',)}
