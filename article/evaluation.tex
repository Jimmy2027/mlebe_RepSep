...
\py{pytex_subfigs(
        ...
        caption='\\textbf{SAMRI processed data has near-optimal conservation of volume compared to the Legacy workflow.} 
...
\py{pytex_subfigs(
        ...
        caption='\\textbf{Significance from functional analysis does not indicate registration quality.} 
        Mean per-animal significance distributions. Coloured patch width estimates distribution density, while continuous markers indicate the sample mean and dashed markers indicate the inner quartiles.',
        label='fig:ms',)}
\end{sansmath}
...
\py{pytex_fig('scripts/variance_catplot.py',
        ...
        label='fig:variance',
        caption='
                \\textbf{SAMRI decreases the across session variance per subject compared to the Legacy workflow.} 
                Swarmplots illustrate the variance in similarity across sessions for each subject.
                ',
        multicol=True,
        )}
\end{sansmath}
...
\py{pytex_subfigs(
        ...
        caption='
                \\textbf{SAMRI does not increase spatial smoothing, thereby increasing locality of functional analysis results.} 
                Smoothness change relative to the original smoothness.
                Coloured patch width estimates distribution density, while continuous markers indicate the sample mean and dashed markers indicate the inner quartiles.
                ',
        label='fig:scf',
        )}
\end{sansmath}
...
Since fMRI data usually doesn't have a gaussian-shaped spatial AutoCorrelation (ACF) (\cite{eklund2016cluster}, \cite{cox2017fmri}), we fit the following function to the data in order to estimate the FWHM using AFNI \cite{cox1996afni}.
...
Looking at \cref{fig:scf}, we find that the SCF is significantly altered by the Legacy ($p= $\py{pytex_printonly('scripts/scf_t_legacy.py')}) as well as the Generic work flow ($p= $\py{pytex_printonly('scripts/scf_t_generic.py')}). We note though, while the Legacy workflow massively inflates the SCF (\py{boilerplate.meanVar('mean', Processing='Legacy')}$\pm$\py{boilerplate.meanVar('sd', Processing='Legacy')}), the Generic only slightly increases the SCF (\py{boilerplate.meanVar('mean', Processing='Generic')}$\pm$\py{boilerplate.meanVar('sd', Processing='Generic')}).
Moreover the Legacy has a substantial increas in variance compared to the Generic workflow across conditions (\py{boilerplate.varianceratio(df_path='data/smoothness_data.csv',dependent_variable='Smoothness Change Factor', max_len=3, template='Legacy')}-fold for the Legacy template and \py{boilerplate.varianceratio(df_path='data/smoothness_data.csv',dependent_variable='Smoothness Change Factor', template='Generic', max_len=3)}-fold for the Generic template).
Overall the SCF is significantly dependend on the processing workflow (\py{boilerplate.fstatistic_smoothness('Processing', condensed=True)}), the template (\py{boilerplate.fstatistic_smoothness('Template', condensed=True)}) and their interaction (\py{boilerplate.fstatistic_smoothness('Processing:Template', condensed=True)}).
