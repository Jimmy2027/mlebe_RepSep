...
Since individual brains should stay rather similar across sessions (no physicial intervention provided), more variation should be found looking across the different subjects of the study. 
\cref{fig:variance} illustrates this effect across the workflows legacy and generic and the similarity measures CC, MI, GC. 
We can now systematically compare the legacy and generic SAMRI workflow using type 3 ANOVA across the different metrics (CC, MI, GC). 
...
Thus we introduce, similar to the Volume metric, a smoothness change factor (SCF), that normalizes the smoothness of the registered images to the original images. 
