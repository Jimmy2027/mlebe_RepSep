...
The volume is then reshaped into \py{boilerplate.get_training_shape('triple')} by first zero-padding the smaller x-y dimension to the same size as the bigger one.
This is done to conserve the ratio of the image.
The z-axis is zero-padded to \py{boilerplate.get_training_shape(type=2, idx=True)}.
The scan is then reshaped into \py{boilerplate.get_training_shape(type='triple')} using the function \textcolor{mg}{\texttt{$cv2.resize$}} from the opencv python package \cite{noauthor_opencv-python_nodate}.
...
Since the classifier was trained to predict on images of shape \py{boilerplate.get_training_shape('triple')}, the input needs to be reshaped.
