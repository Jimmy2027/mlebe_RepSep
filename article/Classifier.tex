...
\py{pytex_fig('scripts/classifier/show_blacklist.py',
        ...
        caption='
                ...
                Examples from the slices that were excluded from the data set. The mask is shown in blue, on top of the brian image.
                ',
        multicol=True,
        )}
\end{sansmath}
...
It is then reshaped into (128, 128) by first zero-padding the smaller dimension to the same size of the bigger one and then reshaping the image into 128 using the function \textcolor{mg}{\texttt{$cv2.resize$}} from the opencv python package. TODO:cite
